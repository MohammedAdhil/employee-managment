...
function Home() {
  ...
  return (
    <>
    <h1 className='text-center text-warning p-5 mt-3'>Employee Management System</h1>
      ...
      <tbody>
        {
            ...
            employee.map(item=>(
                <tr>
                    ...
                    <td>
                      <Link to='/edit'>
                    <Button onClick={()=>handleEdit(item.id,item.name,item.age,item.desig,item.salary)} className='ms-2 me-2' variant="primary"><i class="fa-solid fa-user-pen"></i></Button>
                    </Link>
                    <Button onClick={()=>handleDelete(item.id)} variant="danger"><i class="fa-solid fa-trash-can"></i></Button>{' '}</td>
                </tr>
            ))    
            :'no table data'
        }
